...
Between the two programs, SPAdes was more user-friendly than Velvet. It took significantly less time to run, and was also able to run multiple threads at a time while Velvet could not. SPAdes also recorded a drastically longer genome size compared to the velvet genome size. This could either be a result of the program having a more efficient assembly, or an error, as the Velvet genome size is more comparable to the two reference genomes. Despite this gap, there is no obvioius difference between each type of graph for the programs, suggesting that both programs successfully assembled the tardigrade genome.
